...
const ifEmoji   = (text) => {
	const theEmoji = emoji.get(text);
	return theEmoji ? theEmoji + ' ' : '';
};
const cardIntro = (card) => `${card.name} (${ifEmoji(card.slot)}${card.slot}${limited(card.limited)}${unique(card.unique)}): ${points(card.points)}`;
